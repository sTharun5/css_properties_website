...
**How it works:**
When you add \`display: grid\` to an element, it becomes a grid container. Unlike Flexbox where the items mostly control their own size, Grid is controlled by the container. You set up a skeleton of rows and columns on the parent, and then drop items into the cells you created.
...
<span></span>
**When to use it:**
Normally, you don't need this. The web is meant to scroll down, so letting rows size themselves automatically is usually best. You only use \`grid-template-rows\` when you want to lock the height of your app to exactly 100vh (full screen) and stop it from scrolling.
<span></span>
...
You can force every new automatically-created row to be exactly 200px tall by setting \`grid-auto-rows: 200px\`. A better trick is \`minmax(100px, auto)\` — it guarantees the rows are at least 100px tall for a neat look, but lets them stretch if someone types a very long paragraph.`,
 ...
 applies: 'container',
 description: `Aligns every item horizontally inside its own grid cell track. <span></span>
**The default stretch:**
...
• \`auto-fill\` will create three blank columns to the right. • \`auto-fit\` collapses those empty columns and lets your two items stretch all the way across.`,
 ...
 applies: 'container',
 description: `Controls how the grid automatically places items when you haven't given them specific coordinates. <span></span>
**Row vs Column:**
...
export const gridPatterns = [
 ...
 {
 name: ' Heavy Asymmetrical Masonry',
